...
import org.yaml.snakeyaml.emitter.EmitterConstants.DEFAULT_TAG_PREFIXES
import org.yaml.snakeyaml.emitter.EmitterConstants.ESCAPE_REPLACEMENTS
import org.yaml.snakeyaml.emitter.EmitterConstants.HANDLE_FORMAT
import org.yaml.snakeyaml.emitter.EmitterConstants.MAX_INDENT
import org.yaml.snakeyaml.emitter.EmitterConstants.MIN_INDENT
import org.yaml.snakeyaml.emitter.EmitterConstants.SPACE
import org.yaml.snakeyaml.emitter.EmitterConstants.prepareAnchor
import org.yaml.snakeyaml.error.YAMLException
import org.yaml.snakeyaml.events.*
import org.yaml.snakeyaml.mppio.appendChars
import org.yaml.snakeyaml.mppio.charCount
import org.yaml.snakeyaml.mppio.codePointAt
import org.yaml.snakeyaml.mppio.toCodePoint
import org.yaml.snakeyaml.reader.StreamReader
import org.yaml.snakeyaml.scanner.Constant
import org.yaml.snakeyaml.scanner.Constant.Companion.LINEBR
...
class Emitter(// The stream should have the methods `write` and possibly `flush`.
    private val stream: Appendable, opts: DumperOptions
) : Emitable {
    ...
    private inner class ExpectDocumentStart(private val first: Boolean) : EmitterState {
        ...
        override fun expect() {
            if (event is DocumentStartEvent) {
                ...
                if (ev.tags != null) {
                    val handles = (ev.tags.keys).sorted()
    ...
    private fun checkEmptyDocument(): Boolean {
        ...
        if (event is ScalarEvent) {
            val e = event
            return e.anchor == null && e.tag == null && e.implicit != null && e.value.length == 0
        }
        return false
    }
    ...
    private fun chooseScalarStyle(): DumperOptions.ScalarStyle? {
        ...
        if (!ev.isPlain && ev.scalarStyle == DumperOptions.ScalarStyle.DOUBLE_QUOTED || this.canonical) {
    ...
    private fun processScalar() {
        ...
        if (style == null) {
            ...
            when (style) {
                DumperOptions.ScalarStyle.DOUBLE_QUOTED -> writeDoubleQuoted(analysis.scalar, split)
                DumperOptions.ScalarStyle.SINGLE_QUOTED -> writeSingleQuoted(analysis.scalar, split)
                DumperOptions.ScalarStyle.FOLDED -> writeFolded(analysis.scalar, split)
                DumperOptions.ScalarStyle.LITERAL -> writeLiteral(analysis.scalar)
    ...
    private fun analyzeScalar(scalar: String): ScalarAnalysis {
        ...
        while (index < scalar.length) {
            ...
            val isLineBreak: Boolean = LINEBR.has(c)
            if (isLineBreak) {
                lineBreaks = true
            }
            if (!(c == '\n'.code || (c in 0x20..0x7E))) {
                when (c) {
                    0x85, in 0xA0..0xD7FF, in 0xE000..0xFFFD, in 0x10000..0x10FFFF -> {
                        // unicode is used
                        if (!allowUnicode) {
    ...
    private fun writeLineBreak(data: String?) {
        ...
        column = 0
        if (data == null) {
            stream.appendChars(bestLineBreak)
        } else {
            stream.append(data)
        }
    }
            ...
            if (spaces) {
                if (ch.code == 0 || ch != ' ') {
                    if (start + 1 == end && column > bestWidth && split && start != 0 && end != text.length) {
                        ...
                        val len = end - start
                        column += len
                        stream.appendChars(text.toCharArray(), start, len)
                    }
                    start = end
                }
            } else if (breaks) {
                if (ch.code == 0 || LINEBR.hasNo(ch.code)) {
                    if (text[start] == '\n') {
                        writeLineBreak(null)
                    }
        ...
        while (end <= text.length) {
            ...
            if (ch == null || "\"\\\u0085\u2028\u2029\uFEFF".indexOf(ch) != -1 || ch !in '\u0020'..'\u007E') {
                ...
                if (ch != null) {
                    ...
                    data = if (ESCAPE_REPLACEMENTS.containsKey(ch)) {
                    ...
                    } else if (!allowUnicode || !StreamReader.isPrintable(ch.code)) {
                        ...
                        if (ch <= '\u00FF') {
                            ...
                            if (end + 1 < text.length) {
                                val ch2 = text[++end]
                                val s = "000" + Char.toCodePoint(ch, ch2).toLong().toString(16)
        ...
        if (emitComments) {
            ...
            for (commentLine in commentLines) {
                if (commentLine.commentType != CommentType.BLANK_LINE) {
                    ...
                    stream.append(commentLine.value)
                    writeLineBreak(null)
                } else {
                    writeLineBreak(null)
                    writeIndent()
    ...
    private fun determineBlockHints(text: String): String {
        ...
        if (LINEBR.has(text[0].code, " ")) {
            hints.append(bestIndent)
        }
        val ch1 = text[text.length - 1]
        if (LINEBR.hasNo(ch1.code)) {
            hints.append("-")
        } else if (text.length == 1 || LINEBR.has(text[text.length - 2].code)) {
            hints.append("+")
        }
    ...
    fun writeLiteral(text: String) {
        ...
        while (end <= text.length) {
            ...
            if (breaks) {
                ...
                if (ch.code == 0 || LINEBR.has(ch.code)) {
                    stream.appendChars(text.toCharArray(), start, end - start)
                    if (ch.code == 0) {
                        writeLineBreak(null)
                    }
                    start = end
