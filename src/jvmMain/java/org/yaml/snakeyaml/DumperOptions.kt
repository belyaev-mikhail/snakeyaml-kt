...
class DumperOptions {
    ...
    enum class FlowStyle(val styleBoolean: Boolean?) {
        FLOW(java.lang.Boolean.TRUE), BLOCK(java.lang.Boolean.FALSE), AUTO(null);
    ...
    enum class LineBreak(val string: String) {
        ...
        companion object {
            val platformLineBreak: LineBreak
                get() {
                    val platformLineBreak = System.getProperty("line.separator")
                    for (lb in values()) {
    ...
    var indent = 2
        set(indent) {
            if (indent < Emitter.MIN_INDENT) {
                throw YAMLException("Indent must be at least " + Emitter.MIN_INDENT)
            }
            if (indent > Emitter.MAX_INDENT) {
                throw YAMLException("Indent must be at most " + Emitter.MAX_INDENT)
            }
            field = indent
        }
    ...
    var indicatorIndent = 0
        set(indicatorIndent) {
            ...
            if (indicatorIndent > Emitter.MAX_INDENT - 1) {
                throw YAMLException("Indicator indent must be at most Emitter.MAX_INDENT-1: " + (Emitter.MAX_INDENT - 1))
            }
