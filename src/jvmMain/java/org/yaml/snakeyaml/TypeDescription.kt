 ...
 */
class TypeDescription @JvmOverloads constructor(
    /**
     ...
     */
    @set:Deprecated("it will be removed because it is not used") var tag: Tag = Tag(
        type
    ...
    @Transient
    private var propertyUtils: PropertyUtils? = null

    @Transient
    private var delegatesChecked = false
    private var properties = mutableMapOf<String, PropertySubstitute>()
    protected var excludes = mutableSetOf<String>()
    public var includes: Array<String>? = null
        public get() = field
        public set(value) { field = if (value ==  null || value.isEmpty()) null else value }
    ...
    private fun checkDelegates() {
        ...
        for (p in values) {
            try {
                p.setDelegate(discoverProperty(p.name))
    ...
    fun setPropertyUtils(propertyUtils: PropertyUtils?) {
        this.propertyUtils = propertyUtils
    }

    /* begin: Representer */
//    fun setIncludes(vararg propNames: String?) {
//        includes = if (propNames != null && propNames.size > 0) propNames else null
//    }
    ...
    fun getProperties(): Set<Property?>? {
        ...
        if (propertyUtils != null) {
            ...
            for (property in properties.values) {
                if (!excludes.contains(property.name) && property.isReadable) {
                    dumpProperties?.add(property)
                }
            }
            for (property in readableProps) {
    ...
    fun newInstance(propertyName: String?, node: Node?): Any? {
        return null
    }
     ...
     */
    fun finalizeConstruction(obj: Any): Any {
        return obj
    }

    companion object {
        private val log = Logger
            .getLogger(TypeDescription::class.java.getPackage().name)
    }
}