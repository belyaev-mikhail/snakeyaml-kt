...
open class Yaml constructor(
     ...
     */
    var name: String
    var constructor: BaseConstructor
    var representer: Representer
    protected var dumperOptions: DumperOptions
    ...
    ) : this(constructor, representer, dumperOptions, loadingConfig = loaderOptions) {
    ...
    init {
        ...
        representer.propertyUtils!!.allowReadOnlyProperties = dumperOptions.isAllowReadOnlyProperties
    ...
    fun dumpAsMap(data: Any): String {
        return dumpAs(data, Tag.Companion.MAP, DumperOptions.FlowStyle.BLOCK)
    }
    ...
    fun <T> load(yaml: String): T? {
        return loadFromReader(StreamReader(yaml), Any::class.javaObjectType) as T?
    }
    ...
    fun <T> load(io: InputStream?): T? {
        return loadFromReader(StreamReader(UnicodeReader(io)), Any::class.javaObjectType) as T?
    }
    ...
    open fun <T> load(io: Reader): T? {
        return loadFromReader(StreamReader(io), Any::class.javaObjectType) as T?
    }
     ...
     * sequence
     */
    open fun loadAll(yaml: Reader): Iterable<Any> {
        val composer = Composer(
            ParserImpl(
                StreamReader(yaml),
                loadingConfig.isProcessComments
    ...
    fun setBeanAccess(beanAccess: BeanAccess) {
        constructor.propertyUtils.beanAccess = beanAccess
        representer.propertyUtils!!.beanAccess = beanAccess
    }
    ...
    companion object {
        private fun initDumperOptions(representer: Representer): DumperOptions {
            ...
            dumperOptions.isAllowReadOnlyProperties = representer.propertyUtils!!.allowReadOnlyProperties
