...
abstract class BaseConstructor @JvmOverloads constructor(loadingConfig: LoaderOptions = LoaderOptions()) {
     ...
     */
    @JvmField
    protected val yamlClassConstructors: MutableMap<NodeId, Construct> = EnumMap(NodeId::class.javaObjectType)
    ...
    private var propertyUtils__: PropertyUtils? = null
    var propertyUtils: PropertyUtils
        get() {
            if (propertyUtils__ == null) {
                propertyUtils__ = PropertyUtils()
            }
            return propertyUtils__!!
        }
        set(propertyUtils) {
            propertyUtils__ = propertyUtils
            ...
            for (typeDescription in tds) {
                typeDescription.propertyUtils = propertyUtils
            }
        }
    ...
    init {
        ...
        typeDefinitions[SortedMap::class.javaObjectType] = TypeDescription(
            SortedMap::class.javaObjectType, Tag.OMAP,
            TreeMap::class.javaObjectType
        )
        typeDefinitions[SortedSet::class.javaObjectType] = TypeDescription(
            SortedSet::class.javaObjectType, Tag.SET,
            TreeSet::class.javaObjectType
        )
    ...
    fun getSingleData(type: Class<*>): Any? {
        ...
        return if (node != null && Tag.NULL != node.tag) {
            if (Any::class.javaObjectType != type) {
                node.tag = (Tag(type))
    ...
    fun constructScalar(node: ScalarNode): String? {
        return node.value
    }

    // >>>> DEFAULTS >>>>
    protected open fun createDefaultList(initSize: Int): MutableList<Any?> {
    ...
    protected open fun createDefaultMap(initSize: Int): MutableMap<Any?, Any?> {
    ...
    protected fun newInstance(node: Node): Any {
        return try {
            newInstance(Any::class.javaObjectType, node)
    ...
    protected fun newInstance(ancestor: Class<*>, node: Node): Any {
        return newInstance(ancestor, node, true)
    }

    @Throws(InstantiationException::class)
    protected fun newInstance(ancestor: Class<*>, node: Node, tryDefault: Boolean): Any {
        val type = node.type
    ...
    protected fun newSet(node: CollectionNode<*>): MutableSet<Any?> {
        return try {
            newInstance(MutableSet::class.javaObjectType, node) as MutableSet<Any?>
        } catch (e: InstantiationException) {
    ...
    protected fun newList(node: SequenceNode): MutableList<Any?> {
        return try {
            newInstance(MutableList::class.javaObjectType, node) as MutableList<Any?>
        } catch (e: InstantiationException) {
    ...
    protected fun newMap(node: MappingNode): MutableMap<Any?, Any?> {
        return try {
            newInstance(MutableMap::class.javaObjectType, node) as MutableMap<Any?, Any?>
        } catch (e: InstantiationException) {
    ...
    fun constructSequence(node: SequenceNode): List<Any?> {
    ...
    protected fun constructArrayStep2(node: SequenceNode, array: Any): Any {
        ...
        for (child in node.value) {
            ...
            if (child.type == Any::class.javaObjectType) {
            ...
            if (componentType.isPrimitive) {
                ...
                if (Byte::class.javaPrimitiveType == componentType) {
                    ...
                    Array.setChar(array, index, (value as Char).toChar())
    ...
    fun addTypeDescription(definition: TypeDescription?): TypeDescription? {
        ...
        definition.propertyUtils = propertyUtils
