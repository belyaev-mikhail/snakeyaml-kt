 ...
 */
open class SafeConstructor @JvmOverloads constructor(loadingConfig: LoaderOptions = LoaderOptions()) :
    ...
    protected fun processDuplicateKeys(node: MappingNode, forceStringKeys: Boolean = false) {
        ...
        val keys: MutableMap<Any?, Int> = HashMap(
            nodeValue!!.size
        )
        ...
        for (tuple in nodeValue) {
            ...
            if (keyNode!!.tag != Tag.Companion.MERGE) {
                if (forceStringKeys) {
                    if (keyNode is ScalarNode) {
                        keyNode.type = (String::class.javaObjectType)
                        keyNode.tag = Tag.Companion.STR
                    } else {
                        throw YAMLException(
    ...
    private fun mergeNode(
        ...
        val iter: MutableIterator<NodeTuple> = node.value.iterator()
        while (iter.hasNext()) {
    ...
    inner class ConstructYamlBool : AbstractConstruct() {
        override fun construct(node: Node?): Any? {
            val `val` = constructScalar((node as ScalarNode)) as String
            return BOOL_VALUES[`val`.lowercase(Locale.getDefault())]
        }
    }
    ...
    inner class ConstructYamlBinary : AbstractConstruct() {
        override fun construct(node: Node?): Any {
            ...
            val noWhiteSpaces =
                constructScalar((node as ScalarNode)).toString().replace(
                    "\\s".toRegex(),
                    ""
                )
    ...
    init {
        yamlConstructors[Tag.Companion.NULL] = ConstructYamlNull()
        yamlConstructors[Tag.Companion.BOOL] = ConstructYamlBool()
        yamlConstructors[Tag.Companion.INT] = ConstructYamlInt()
        yamlConstructors[Tag.Companion.FLOAT] = ConstructYamlFloat()
        yamlConstructors[Tag.Companion.BINARY] =
            ConstructYamlBinary()
        yamlConstructors[Tag.Companion.TIMESTAMP] =
            ConstructYamlTimestamp()
        yamlConstructors[Tag.Companion.OMAP] = ConstructYamlOmap()
        yamlConstructors[Tag.Companion.PAIRS] = ConstructYamlPairs()
        yamlConstructors[Tag.Companion.SET] = ConstructYamlSet()
        yamlConstructors[Tag.Companion.STR] = ConstructYamlStr()
        yamlConstructors[Tag.Companion.SEQ] =
            ConstructYamlSeq()
        yamlConstructors[Tag.Companion.MAP] =
            ConstructYamlMap()
        yamlConstructors[null] = undefinedConstructor
        yamlClassConstructors[NodeId.scalar] =
            undefinedConstructor
        yamlClassConstructors[NodeId.sequence] = undefinedConstructor
        yamlClassConstructors[NodeId.mapping] =
            undefinedConstructor
    }

    open class ConstructYamlTimestamp : AbstractConstruct() {
    ...
    inner class ConstructYamlOmap : AbstractConstruct() {
        override fun construct(node: Node?): Any {
            ...
            node ?: throw NullPointerException("node is null")
        ...
        init {
            ...
            for (radix in radixList) {
                RADIX_MAX[radix] = intArrayOf(
                    maxLen(Int.MAX_VALUE, radix), maxLen(
                        Long.MAX_VALUE, radix
                    )
                )
            }
        }
