...
object Base64Coder {
     ...
     */
    fun encodeLines(`in`: ByteArray): String {
        return encodeLines(`in`, 0, `in`.size, 76, systemLineSeparator)
    }
     ...
     */
    fun encodeLines(
        `in`: ByteArray, iOff: Int, iLen: Int, lineLen: Int,
     ...
     * @return A character array containing the Base64 encoded data.
     */
    fun encode(`in`: ByteArray, iOff: Int, iLen: Int): CharArray {
     ...
     * If the input is not valid Base64 encoded data.
     */
    fun decode(`in`: CharArray, iOff: Int, iLen: Int): ByteArray {
        var iLen = iLen
        ...
        while (ip < iEnd) {
            val i0 = `in`[ip++].code
            val i1 = `in`[ip++].code
            val i2 = if (ip < iEnd) `in`[ip++].code else 'A'.code
            val i3 = if (ip < iEnd) `in`[ip++].code else 'A'.code
