...
abstract class GenericProperty(name: String?, aClass: Class<*>, private val genType: Type?) : Property(name, aClass) {
    ...
    private var actualClasses: Array<Class<*>>?
            ...
            if (!actualClassesChecked) {
                if (genType is ParameterizedType) {
                    ...
                    if (actualTypeArguments.size > 0) {
                        actualClasses = arrayOfNulls(actualTypeArguments.size)
                        for (i in actualTypeArguments.indices) {
                            if (actualTypeArguments[i] is Class<*>) {
                                actualClasses.get(i) = actualTypeArguments[i] as Class<*>
                            } else if (actualTypeArguments[i] is ParameterizedType) {
                                actualClasses.get(i) = (actualTypeArguments[i] as ParameterizedType)
                                    .rawType as Class<*>
                                ...
                                if (componentType is Class<*>) {
                                    actualClasses.get(i) = java.lang.reflect.Array.newInstance(componentType, 0)
                                        .javaClass
                                } else {
                                    actualClasses = null
                                    break
                    ...
                    // required for IcedTea6
                    if (genType.isArray) {
                        actualClasses = arrayOfNulls(1)
                        actualClasses.get(0) = type.componentType
                    }
                }
