...
open class PropertyUtils internal constructor(private val platformFeatureDetector: PlatformFeatureDetector) {
    ...
    var beanAccess = BeanAccess.DEFAULT
        get(): BeanAccess {
            return field
        }
        set(beanAccess: BeanAccess) {
            require(!(platformFeatureDetector.isRunningOnAndroid == true && beanAccess != BeanAccess.FIELD)) { "JVM is Android - only BeanAccess.FIELD is available" }
            if (field != beanAccess) {
                field = beanAccess
    ...
    public var allowReadOnlyProperties = false
        set(allowReadOnlyProperties: Boolean) {
            if (field != allowReadOnlyProperties) {
                field = allowReadOnlyProperties
                readableProperties.clear()
            }
        }
    ...
    init {
         ...
         */if (platformFeatureDetector.isRunningOnAndroid == true) {
    ...
    protected open fun createPropertySet(type: Class<out Any?>, bAccess: BeanAccess): MutableSet<Property> {
