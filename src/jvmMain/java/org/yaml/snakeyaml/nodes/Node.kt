 ...
 *
 */
abstract class Node(tag: Tag?, startMark: Mark?, endMark: Mark?) {
    internal var tag: Tag? = null
    val startMark: Mark?
    open var endMark: Mark? = null
        protected set
    internal var type: Class<out Any>
    ...
    var isResolved: Boolean
        protected set
    public var useClassConstructor: Boolean?
    ...
    fun setTag(tag: Tag?) {
        if (tag == null) {
            throw NullPointerException("tag in a Node is required.")
        }
        this.tag = tag
    }
    ...
    fun setType(type: Class<out Any>) {
    ...
    fun useClassConstructor(): Boolean {
        return if (useClassConstructor == null) {
            if (!tag!!.isSecondary && isResolved && Any::class.java != type
                && tag != Tag.Companion.NULL
            ) {
                true
        ...
        } else useClassConstructor as Boolean
    }
}