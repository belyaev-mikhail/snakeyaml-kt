...
abstract class BaseRepresenter {
    ...
    protected val multiRepresenters: Map<Class<*>?, Represent> = LinkedHashMap()
    protected var defaultScalarStyle: DumperOptions.ScalarStyle? = null //not explicitly defined
    ...
    protected val representedObjects: MutableMap<Any?, Node> = object : IdentityHashMap<Any, Node?>() {
        private static
        val serialVersionUID = -5576159264232131854L
    ...
    protected fun representScalar(
        tag: Tag?,
        value: String?,
        style: DumperOptions.ScalarStyle? = null
    ): Node {
        var style = style
        if (style == null) {
            style = defaultScalarStyle
        }
        return ScalarNode(tag, value, null, null, style)
    }
    ...
    fun setDefaultScalarStyle(defaultStyle: DumperOptions.ScalarStyle?) {
        defaultScalarStyle = defaultStyle
    }

    fun getDefaultScalarStyle(): DumperOptions.ScalarStyle {
        return if (defaultScalarStyle == null) {
            DumperOptions.ScalarStyle.PLAIN
        } else defaultScalarStyle
    }
}