 ...
 * Represent JavaBeans
 */
open class Representer : SafeRepresenter {
    ...
    fun addTypeDescription(td: TypeDescription): TypeDescription? {
        ...
        td.propertyUtils = propertyUtils
     ...
     */
    protected open fun representJavaBean(properties: Set<Property>, javaBean: Any): MappingNode? {
     ...
     * property
     */
    protected open fun representJavaBeanProperty(
        ...
        if (propertyValue != null && !hasAlias) {
            ...
            if (customTag == null) {
                if (nodeId == NodeId.scalar) {
                    ...
                    if (property.type != Enum::class.javaObjectType) {
                    ...
                    if (nodeId == NodeId.mapping) {
                        if (property.type == propertyValue.javaClass) {
    ...
    private fun resetTag(type: Class<out Any?>, node: Node) {
        val tag = node.tag
        if (tag!!.matches(type)) {
            if (Enum::class.javaObjectType.isAssignableFrom(type)) {
     ...
     */
    protected open fun getProperties(type: Class<out Any?>): Set<Property>? {
        return if (typeDefinitions.containsKey(type)) {
            typeDefinitions[type]!!.getProperties()
        } else propertyUtils!!.getProperties(type)
    }
}
