...
open class SafeRepresenter @JvmOverloads constructor(options: DumperOptions = DumperOptions()) : BaseRepresenter() {
    ...
    init {
        ...
        representers[String::class.javaObjectType] = RepresentString()
        representers[Boolean::class.javaObjectType] = RepresentBoolean()
        representers[Char::class.javaObjectType] = RepresentString()
        representers[UUID::class.javaObjectType] = RepresentUuid()
        representers[ByteArray::class.javaObjectType] = RepresentByteArray()
        val primitiveArray: Represent = RepresentPrimitiveArray()
        representers[ShortArray::class.javaObjectType] = primitiveArray
        representers[IntArray::class.javaObjectType] = primitiveArray
        representers[LongArray::class.javaObjectType] = primitiveArray
        representers[FloatArray::class.javaObjectType] = primitiveArray
        representers[DoubleArray::class.javaObjectType] = primitiveArray
        representers[CharArray::class.javaObjectType] = primitiveArray
        representers[BooleanArray::class.javaObjectType] = primitiveArray
        multiRepresenters[Number::class.javaObjectType] = RepresentNumber()
        multiRepresenters[MutableList::class.javaObjectType] =
            RepresentList()
        multiRepresenters[MutableMap::class.javaObjectType] = RepresentMap()
        multiRepresenters[MutableSet::class.javaObjectType] = RepresentSet()
        multiRepresenters[MutableIterator::class.javaObjectType] =
            RepresentIterator()
        multiRepresenters[arrayOfNulls<Any>(0).javaClass] =
            RepresentArray()
        multiRepresenters[Date::class.javaObjectType] = RepresentDate()
        multiRepresenters[Enum::class.javaObjectType] = RepresentEnum()
        multiRepresenters[Calendar::class.javaObjectType] = RepresentDate()
        classTags = HashMap()
        nonPrintableStyle = options.nonPrintableStyle
    }

    protected open inner class RepresentString : Represent {
        override fun representData(data: Any?): Node {
            var tag: Tag = Tag.Companion.STR
            var style: DumperOptions.ScalarStyle? = null //not defined
            var value = data.toString()
            if (nonPrintableStyle == DumperOptions.NonPrintableStyle.BINARY && !StreamReader.Companion.isPrintable(value)) {
                ...
                binary = try {
                    ...
                    val checkValue = String(bytes, Charsets.UTF_8)
                    if (checkValue != value) {
    ...
    protected open inner class RepresentDate : Represent {
        override fun representData(data: Any?): Node? {
    ...
    protected inner class RepresentByteArray : Represent {
        override fun representData(data: Any?): Node? {
            val binary = Base64Coder.encode(data as ByteArray)
    ...
    protected inner class RepresentUuid : Represent {
        override fun representData(data: Any?): Node? {
            return representScalar(getTag(data!!.javaClass, Tag(UUID::class.javaObjectType)), data.toString())
        }
    }
