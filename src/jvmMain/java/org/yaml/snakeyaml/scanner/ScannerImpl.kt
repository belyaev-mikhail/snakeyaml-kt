...
class ScannerImpl(private val reader: StreamReader) : Scanner {
    ...
    private fun fetchMoreTokens() {
        ...
        val c = reader.peek()
        when (c) {
            // CONVERT_TODO: make sure this becomes a linear switch
            '\u0000'.code -> {
        ...
        var chRepresentation = escapeChar(String(Character.toChars(c)))
    ...
    private fun checkDocumentStart(): Boolean {
        // DOCUMENT-START: ^ '---' (' '|'\n')
        if (reader.column == 0) {
            if ("---" == reader.prefix(3) && Constant.Companion.NULL_BL_T_LINEBR.has(reader.peek(3))) {
                return true
            }
        }
        return false
    ...
    private fun checkBlockEntry(): Boolean {
        // BLOCK-ENTRY: '-' (' '|'\n')
        return Constant.Companion.NULL_BL_T_LINEBR.has(reader.peek(1))
    }
    ...
    private fun checkKey(): Boolean {
        ...
        return if (flowLevel != 0) {
            ...
            // KEY(block context): '?' (' '|'\n')
            Constant.Companion.NULL_BL_T_LINEBR.has(reader.peek(1))
        }
    }
    ...
    private fun checkPlain(): Boolean {
        ...
        return (
                (Constant.NULL_BL_T_LINEBR.hasNo(c, "-?:,[]{}#&*!|>\'\"%@`")
                        || Constant.NULL_BL_T_LINEBR.hasNo(reader.peek(1)))
                        && (c == '-'.code || flowLevel == 0) && "?:".indexOf(c.toChar()) != -1)
    }
    // Scanners.
    /**
     * <pre>
    ...
    private fun scanComment(type: CommentType): CommentToken {
        ...
        reader.forward()
        var length = 0
        while (Constant.Companion.NULL_OR_LINEBR.hasNo(reader.peek(length))) {
            length++
        }
    ...
    private fun scanDirective(): List<Token> {
        ...
        if ("YAML" == name) {
            ...
            endMark = reader.mark
            var ff = 0
            while (Constant.Companion.NULL_OR_LINEBR.hasNo(reader.peek(ff))) {
                ff++
            }
            if (ff > 0) {
                reader.forward(ff)
    ...
    private fun scanDirectiveName(startMark: Mark?): String? {
        ...
        while (Constant.Companion.ALPHA.has(c)) {
            length++
        ...
        val value = reader.prefixForward(length)
        c = reader.peek()
        if (Constant.Companion.NULL_BL_LINEBR.hasNo(c)) {
            val s = String(Character.toChars(c))
            throw ScannerException(
                "while scanning a directive", startMark,
                "expected alphabetic or numeric character, but found " + s + "(" + c
    ...
    private fun scanAnchor(isAnchor: Boolean): Token {
        ...
        var length = 0
        var c = reader.peek(length)
        while (Constant.Companion.NULL_BL_T_LINEBR.hasNo(c, ":,[]{}/.*&")) {
            length++
            c = reader.peek(length)
        }
        if (length == 0) {
        ...
        val value = reader.prefixForward(length)
        c = reader.peek()
        if (Constant.Companion.NULL_BL_T_LINEBR.hasNo(c, "?:,]}%@`")) {
            val s = String(Character.toChars(c))
            throw ScannerException(
                "while scanning an $name", startMark,
                "unexpected character found $s($c)", reader.mark
    ...
    private fun scanTag(): Token {
        ...
        if (c == '<'.code) {
            ...
            reader.forward()
        } else if (Constant.Companion.NULL_BL_T_LINEBR.has(c)) {
            ...
            var length = 1
            var useHandle = false
            while (Constant.Companion.NULL_BL_LINEBR.hasNo(c)) {
                if (c == '!'.code) {
                    useHandle = true
                    break
                }
    ...
    private fun scanBlockScalar(style: Char): List<Token> {
        ...
        // We are done.
        val scalarToken = ScalarToken(chunks.toString(), false, startMark, endMark, createStyle(style))
    ...
    private fun scanBlockScalarIndentation(): Array<Any?> {
        ...
        while (Constant.Companion.LINEBR.has(reader.peek(), " \r")) {
            if (reader.peek() != ' '.code) {
    ...
    private fun scanFlowScalar(style: Char): Token {
        ...
        reader.forward()
        val endMark = reader.mark
        return ScalarToken(chunks.toString(), false, startMark, endMark, createStyle(style))
    }
    ...
    private fun scanFlowScalarNonSpaces(doubleQuoted: Boolean, startMark: Mark?): String {
        ...
        while (true) {
            ...
            var length = 0
            while (Constant.Companion.NULL_BL_T_LINEBR.hasNo(reader.peek(length), "\'\"\\")) {
                length++
            }
            if (length != 0) {
                chunks.append(reader.prefixForward(length))
            ...
            if (!doubleQuoted && c == '\''.code && reader.peek(1) == '\''.code) {
                ...
                reader.forward()
                c = reader.peek()
                if (!Character.isSupplementaryCodePoint(c) && ESCAPE_REPLACEMENTS.containsKey(Character.valueOf(c.toChar()))) {
                    // The character is one of the single-replacement
                    // types; these are replaced with a literal character
                    // from the mapping.
                    chunks.append(ESCAPE_REPLACEMENTS[Character.valueOf(c.toChar())])
                    reader.forward()
                } else if (!Character.isSupplementaryCodePoint(c) && ESCAPE_CODES.containsKey(Character.valueOf(c.toChar()))) {
                    // The character is a multi-digit escape sequence, with
                    // length defined by the value in the ESCAPE_CODES map.
                    length = ESCAPE_CODES[Character.valueOf(c.toChar())]!!
                        .toInt()
                    reader.forward()
    ...
    private fun scanFlowScalarBreaks(startMark: Mark?): String {
        ...
        while (true) {
            ...
            if ("---" == prefix || "..." == prefix
                && Constant.Companion.NULL_BL_T_LINEBR.has(reader.peek(3))
            ) {
                throw ScannerException(
                    "while scanning a quoted scalar", startMark,
    ...
    private fun scanPlain(): Token {
        ...
        while (true) {
            ...
            while (true) {
                c = reader.peek(length)
                if (Constant.Companion.NULL_BL_T_LINEBR.has(c) || c == ':'.code && Constant.Companion.NULL_BL_T_LINEBR.has(
    ...
    private fun atEndOfPlain(): Boolean {
        ...
        run {
            ...
            while (reader.peek(wsLength)
                    .also { c = it } != '\u0000'.code && Constant.Companion.NULL_BL_T_LINEBR.has(c)
            ) {
                wsLength++
                if (!Constant.Companion.LINEBR.has(c) && (c != '\r'.code || reader.peek(wsLength + 1) != '\n'.code) && c != 0xFEFF) {
                    wsColumn++
                } else {
                    wsColumn = 0
                }
        ...
        if (flowLevel == 0) {
            var c: Int
            var extra = 1
            while (reader.peek(wsLength + extra).also { c = it } != 0 && !Constant.Companion.NULL_BL_T_LINEBR.has(c)) {
                if (c == ':'.code && Constant.Companion.NULL_BL_T_LINEBR.has(reader.peek(wsLength + extra + 1))) {
                    return true
                }
                extra++
            }
    ...
    private fun scanPlainSpaces(): String? {
        ...
        if (lineBreak.length != 0) {
            ...
            if ("---" == prefix || "..." == prefix && Constant.Companion.NULL_BL_T_LINEBR.has(
                    reader.peek(3)
                )
            ) {
                return ""
    ...
    private fun scanTagUri(name: String, startMark: Mark?): String {
        ...
        var length = 0
        var c = reader.peek(length)
        while (Constant.Companion.URI_CHARS.has(c)) {
            if (c == '%'.code) {
                chunks.append(reader.prefixForward(length))
                length = 0
    ...
    private fun makeTokenList(vararg tokens: Token?): List<Token> {
        ...
        for (token in tokens) {
            ...
            if (!isParseComments && token is CommentToken) {
                continue
            }
            tokenList.add(token)
        }
    ...
    companion object {
        ...
        init {
            // ASCII null
            ESCAPE_REPLACEMENTS[Character.valueOf('0')] = "\u0000"
            // ASCII bell
            ESCAPE_REPLACEMENTS[Character.valueOf('a')] = "\u0007"
            // ASCII backspace
            ESCAPE_REPLACEMENTS[Character.valueOf('b')] = "\u0008"
            // ASCII horizontal tab
            ESCAPE_REPLACEMENTS[Character.valueOf('t')] = "\u0009"
            // ASCII newline (line feed; &#92;n maps to 0x0A)
            ESCAPE_REPLACEMENTS[Character.valueOf('n')] = "\n"
            // ASCII vertical tab
            ESCAPE_REPLACEMENTS[Character.valueOf('v')] = "\u000B"
            // ASCII form-feed
            ESCAPE_REPLACEMENTS[Character.valueOf('f')] = "\u000C"
            // carriage-return (&#92;r maps to 0x0D)
            ESCAPE_REPLACEMENTS[Character.valueOf('r')] = "\r"
            // ASCII escape character (Esc)
            ESCAPE_REPLACEMENTS[Character.valueOf('e')] = "\u001B"
            // ASCII space
            ESCAPE_REPLACEMENTS[Character.valueOf(' ')] = "\u0020"
            // ASCII double-quote
            ESCAPE_REPLACEMENTS[Character.valueOf('"')] = "\""
            // ASCII backslash
            ESCAPE_REPLACEMENTS[Character.valueOf('\\')] = "\\"
            ...
            ESCAPE_REPLACEMENTS[Character.valueOf('_')] = "\u00A0"
            // Unicode line-separator
            ESCAPE_REPLACEMENTS[Character.valueOf('L')] = "\u2028"
            // Unicode paragraph separator
            ESCAPE_REPLACEMENTS[Character.valueOf('P')] = "\u2029"

            // 8-bit Unicode
            ESCAPE_CODES[Character.valueOf('x')] = 2
            // 16-bit Unicode
            ESCAPE_CODES[Character.valueOf('u')] = 4
            // 32-bit Unicode (Supplementary characters are supported)
            ESCAPE_CODES[Character.valueOf('U')] = 8
        }
    }
}